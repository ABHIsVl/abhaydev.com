...
export default function ContactPopup({ onClose }: ContactPopupProps) {
  ...
  useEffect(() => {
    if (popupRef.current && formContentRef.current) {
      gsap.set(popupRef.current, { y: 50, opacity: 1 });
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4" onClick={handleBackdropClick}>
      <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" aria-hidden="true"></div>
      <div 
        ref={popupRef} 
        className={`border border-border/40 ${theme === 'dark' ? 'bg-card' : 'bg-background'} rounded-xl w-full max-w-md p-8 z-[101] relative shadow-lg`}
